<script lang="ts">
  ...
  function isTooClose(pos1: {top: number, left: number}, pos2: {top: number, left: number}, distance: number) {
  ...
  function getNewPosition(existingLines: LatexLine[]) {
    ...
    do {
      ...
      attempts++;
      if (attempts > maxAttempts) break; // Fallback if we can't find a spot
    } while (existingLines.some(line => isTooClose(newPos, line, minDistance)));
  ...
  onMount(() => {
    interval = setInterval(() => {
      lines = lines.map(line => {
        ...
        } else if (Math.random()<0.03) { // Occasionally reset to a new snippet
...
<div class="bg-latex">
  {#each lines as line (line.key)}
    <span
      ...
      aria-hidden="true"
      >{line.snippet.slice(0, line.progress)}</span>
  {/each}
</div>
